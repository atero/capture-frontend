...
A more realistic [starter script "foo.js"](./dashboard/foo.js) is as follows.
...
See the ["examples"](dashboard/) dir for a number of starter examples using
...
```shell
$ FOO_VERBOSE= node dashboard/foo.js                 # not set
# opts: { _order: [], _args: [] }
# args: []

$ FOO_VERBOSE=0 node dashboard/foo.js                # '0' is false
# opts: { verbose: [ false ],
  _order: [ { key: 'verbose', value: false, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_VERBOSE=1 node dashboard/foo.js                # true
# opts: { verbose: [ true ],
  _order: [ { key: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_VERBOSE=boogabooga node dashboard/foo.js       # true
# opts: { verbose: [ true ],
  _order: [ { key: 'verbose', value: true, from: 'env' } ],
  _args: [] }
# args: []
...
```shell
$ FOO_FILE=data.txt node dashboard/foo.js
# opts: { file: 'data.txt',
...
```shell
$ FOO_TIMEOUT=5000 node dashboard/foo.js
# opts: { timeout: 5000,
  _order: [ { key: 'timeout', value: 5000, from: 'env' } ],
  _args: [] }
# args: []

$ FOO_TIMEOUT=blarg node dashboard/foo.js
...
See [dashboard/ddcompletion.js](dashboard/ddcompletion.js) for a complete
...
- `completionType` (String). Optional. This is used for [Bash
  ...
  following values have meaning:
    ...
    - *Any string FOO for which a `function complete_FOO` Bash function is
      ...
      ["dashboard/ddcompletion.js"](dashboard/ddcompletion.js) for an example.
...
See "dashboard/custom-option-\*.js" for other dashboard.
