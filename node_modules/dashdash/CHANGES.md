...
- [pull #16] Change the `positiveInteger` type to NOT accept zero (0).
  ...
  "dashboard/custom-option-intGteZero.js".  (By Dave Pacheco.)
...
- Support for generating Bash completion files. See the "Bash completion"
  section of the README.md and "dashboard/ddcompletion.js" for an example.
...
- Add the `arrayFlatten` boolean option to `dashdash.addOptionType` used for
  ...
  `opts.foo = ['a', 'b', 'c']`. See
  "[dashboard/custom-option-arrayOfCommaSepString.js](dashboard/custom-option-arrayOfCommaSepString.js)"
  for an example.
...
- [issue #10] Custom option types added with `addOptionType` can specify a
  "default" value. See "dashboard/custom-option-fruit.js".
...
- Add support for adding custom option types. "dashboard/custom-option-duration.js"
...
- [issue #4] Add `date` and `arrayOfDate` option types. They accept these date
  ...
  "2014-03-28T18:35:01.489Z"). See "dashboard/date.js" for an example usage.

        $ node dashboard/date.js -s 2014-01-01 -e $(date +%s)
...
- [Backward incompatible change for boolean envvars] Change the
  ...
  true -- which was quite misleading. Example:
        ...
        $ FOO_VERBOSE=0 node dashboard/foo.js
        # opts: { verbose: [ false ],
